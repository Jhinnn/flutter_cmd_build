...
APP_VERSION=0.3.$COMMIT_COUNT-$APP_PRERELEASE_VERSION+$APP_GIT_HEAD.$BUILD_TIMESTAMP
echo $APP_VERSION
    
buildcmd="flutter build ios --dart-define=NEEDLOG=0 --dart-define=APPENTRYPOINT="$APPENTRYPOINT" --dart-define=APPGITHEAD="$APPGITHEAD"  --dart-define=APPBUILDTIME="$APPBUILDTIME" --dart-define=APPVERSION="$APP_VERSION" --release"
echo $buildcmd
$buildcmd
